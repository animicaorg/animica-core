...
| Free AI inference | `https://animica.dev/v1` | OpenAI-compatible (chat/completions, models), **keyless**, 30 req/min/IP. Capacity is community-GPU: check each model's `serving` flag in `/v1/models` — non-serving models may 503 or queue. `/v1/embeddings` is not currently deployed |
...
- **AI-compute RPC lives under `aicf.*`** (59 methods: submitInferenceJob, jobStatus, estimateJobCost, listProviders…; enumerate via `https://explorer.animica.org/api/rpc/discover`). There is **no `ai.*` namespace** and **no `chain.getHeight`** — use `chain.getHead` and read `height`.
