...
export type VerifyEmailResult =
  ...
  | { ok: false; code: "invalid_token" | "expired_token" | "already_verified"; message: string };
...
export async function verifyEmailToken(pool: PgPoolLike, token: string): Promise<VerifyEmailResult> {
  const tokenHash = hashVerificationToken(token);
...
export async function findUserForVerificationEmail(pool: PgPoolLike, email: string) {
  const result = await pool.query(
    `
