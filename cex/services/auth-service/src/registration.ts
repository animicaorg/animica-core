...
async function withTransaction<T>(pool: PgPoolLike, callback: (client: PgClientLike) => Promise<T>): Promise<T> {
...
async function ensureReferralCode(client: PgClientLike, userId: string): Promise<string> {
...
async function findReferrer(client: PgClientLike, referralCode: string) {
  const result = await client.query(
    `
...
async function ensureAirdropPool(client: PgClientLike) {
  await client.query(
    `
...
async function creditSignupReferralReward(
  client: PgClientLike,
  referral: { id: string; referrerUserId: string; referredUserId: string; code: string }
) {
