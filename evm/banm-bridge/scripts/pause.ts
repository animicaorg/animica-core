...
async function main(): Promise<void> {
  ...
  if (target === "all" || target === "token") await (await token.pause()).wait();
  if (target === "all" || target === "controller") await (await controller.pause()).wait();
  if (target === "all" || target === "vault") await (await vault.pause()).wait();
  if (target === "all" || target === "router") await (await router.pause()).wait();

  console.log(`Paused target: ${target}`);
}
