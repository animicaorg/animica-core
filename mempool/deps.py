...
class StateView:
    ...
    def get_balance(self, address: bytes) -> int:
        ...
        # Fallback: try generic get(("acct", addr_hex, "balance"))
        try:
...
class HeadTracker:
    ...
    def refresh(self) -> HeadInfo:
        """Read the canonical head from BlockDB or core.chain.head helpers."""
...
def _extract_fee_sample(block: Any) -> FeeSample:
    ...
    tip_med = int(median(tips)) if tips else None
...
class RecentFeeStats:
    ...
    def prime_from_chain(self, count: Optional[int] = None) -> int:
        ...
        If `count` is None, use the current window size. Returns blocks ingested.
        """
        if self._db is None:
            return 0
        ...
        # Fallback: walk (head -> get_block_by_hash -> parentHash ...)
        try:
            head = None
            if hasattr(self._db, "get_canonical_head"):
    ...
    def ema_utilization(self, alpha: float = 0.3) -> float:
        ...
        if not self._samples:
            return 0.0
        s0 = self._samples[0]
        z = s0.gas_used / max(1, s0.gas_limit)
    ...
    def snapshot(self) -> Dict[str, Union[int, float, None]]:
        ...
        Keys: base_fee_ema, utilization_ema, median_tip, tx_count_avg.
        """
        return {
